...
import com.example.acupuncture.camera.VisionProcessorBase;
...
//                        .setClassificationMode(FaceDetectorOptions.CLASSIFICATION_MODE_ALL)
    ...
    protected void onSuccess(@NonNull List<Face> faces, @NonNull GraphicOverlay graphicOverlay) {
        for (Face face : faces) {
            graphicOverlay.add(new FaceGraphic(graphicOverlay, face, pos));
    ...
    private static void logExtrasForTesting(Face face) {
        if (face != null) {
            ...
            FaceLandmark test = face.getLandmark(FaceLandmark.RIGHT_EAR);
            if(test != null)
                Log.v(MANUAL_TESTING_LOG, String.format("B8_test_x: %f", test.getPosition().x));
